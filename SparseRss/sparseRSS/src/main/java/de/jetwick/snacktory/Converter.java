        ...
        try {
            ...
            try {
                ...
                if (tmpEnc != null)
                    ...
                    System.out.println("no charset found in first stage");
                    // detect with the help of xml beginning ala encoding="charset"
                    tmpEnc = detectCharset("encoding=", output, in, encoding);
                    if (tmpEnc != null)
                        encoding = tmpEnc;
                    else
                        System.out.println("no charset found in second stage");
                }

                if (!Charset.isSupported(encoding))
                    throw new UnsupportedEncodingException(encoding);
            } catch (UnsupportedEncodingException e) {
            	System.err.println("Using default encoding:" + UTF8
            ...
            while (true) {
                if (bytesRead >= maxBytes) {
                	System.err.println("Maxbyte of " + maxBytes + " exceeded! Maybe html is now broken but try it nevertheless. Url: " + url);
                    break;
                }
        ...
        if (encIndex > 0) {
            ...
            if (lastEncIndex > encIndex + clength && lastEncIndex < encIndex + clength + 40) {
                ...
                try {
                	...
                	System.err.println("Couldn't reset stream to re-read with new encoding " + tmpEnc + " "
                            + ex.toString());
                }
            }
        }
